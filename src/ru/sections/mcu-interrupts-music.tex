...
\index{Музыка!Совмещение двух мелодий на одной Arduino}
...
\index{Музыка!Совмещение двух мелодий на одной Arduino!Таймеры}
...
\index{Музыка!Совмещение двух мелодий на одной Arduino!Музыка на прерываниях}
...
\ref{listing:mcu-music-2-1}, и здесь же объявим массив с мелодией (см. листинг
\ref{listing:mcu-music-2-2}.)
...
\begin{listing}[H]
  \begin{minted}{cpp}
    typedef struct {
      int length; // Количество нот в мелодии.
      int  note_index; // Индекс (номер) ноты.
      // Время начала воспроизведения ноты.
      long note_start_time;
      long note_length; // Длина ноты.
      long note_delay; // Задержка после ноты.
      long current_time; // Текущее время.
      // Время, прошедшее с начала воспроизведения ноты.
      long time_diff;
      long ocr; // Виртуальный регистр OCR.
      long counter; // Счётчик для таймера.
      byte speaker_pin; // Цифровой порт для вывода звука.
...
\begin{listing}[H]
  \begin{minted}{cpp}
    const int SPEAKER_1 = 2; // Порт вывода звука.
    const long CPU_FREQ = 16000000; // Частота процессора (Гц)
    const long CPU_PRESCALER = 8.0; // Предделитель частоты.
    ...
    const long TAKT = (MINUTE / BPM) * 4; // Длина такта (мкс)
  \end{minted}
...
\index{Музыка!Совмещение двух мелодий на одной Arduino!Двухканальный звук}
...
первого прерывания, и \mintinline{cpp}{TIMER1_COMPB_vect} -- для второго.
